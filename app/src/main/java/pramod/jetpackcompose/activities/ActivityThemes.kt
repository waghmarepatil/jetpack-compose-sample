...
import pramod.jetpackcompose.model.Food
import pramod.jetpackcompose.ui.theme.*
    ...
    fun RowFoodItem(food: Food) {
        Card(elevation = 10.dp, modifier = Modifier.padding(top = 20.dp)) {
            Column(
                ...
                Text(text = food.name, style = MaterialTheme.typography.customTitle)

                CompositionLocalProvider(LocalContentAlpha provides (ContentAlpha.disabled)) {
                    Text(text = food.description)//description
                }
            }
        }
    }
